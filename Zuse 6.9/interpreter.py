# FILE: interpreter.py (Version ZUSE 6.9 SMART IMPORT)
import time
import math
import random
import datetime
...
ZUSE_LIBS = {'deutsch', 'english', 'espanol', 'portugues', 'francais', 'italiano', 'portugues'}
...
class Environment:
    ...
    def set(self, name, value):
        ...
        elif self.parent and self.parent.has_recursive(name): self.parent.set(name, value)
...
class ZuseInstance:
    ...
    def get_attr(self, name):
        if name in self._attributes: return self._attributes[name]
        return None
        ...
        while curr_wrapper:
            ...
            if parent_name:
                try:
                    ...
                    if not isinstance(parent_wrapper, ZuseClassWrapper): raise ZuseError(f"'{parent_name}' ist keine Klasse.")
                    curr_wrapper = parent_wrapper
                except ZuseError: return None, None, None
...
class Interpreter:
    def __init__(self, output_callback=print, input_callback=input, safe_mode=False):
        ...
        self.global_env.define("__UMGEBUNG__", "STANDALONE")
    ...
    def interpretiere(self, ast):
        ...
        except ZuseError as e: self.print_out(f"Laufzeitfehler: {e}")
        except RecursionError: self.print_out("Kritischer Fehler: Maximale Rekursionstiefe überschritten.")
            ...
            if typ == 'ZUWEISUNG':
                ...
                if ziel['type'] == 'VARIABLE':
                    ...
                    try:
                        ...
                        if is_global and name in is_global: self.global_env.vars[name] = wert
                ...
                else: raise ZuseError(f"Ungültiges Zuweisungsziel: {ziel['type']}")
                ...
                if node['name'] not in self.global_env.vars: self.global_env.define(node['name'], None)
                ...
                # --- FIX: Zuse-Sprachdateien ignorieren und erlauben ---
                if mod_name in ZUSE_LIBS:
                    # Wir tun einfach nichts, denn der Code wurde schon vom Studio geladen.
                    # Das verhindert den Security-Crash UND den ImportError.
                    return 
                # -------------------------------------------------------
                ...
                if self.safe_mode:
                    ...
                    if not allowed: raise ZuseError(f"Sicherheits-Sperre: Modul '{mod_name}' ist im Lernmodus nicht erlaubt.")
                
                if mod_name in ALLOWED_MODULES: env.define(alias, ALLOWED_MODULES[mod_name])
                else:
                    try:
                        imported_module = importlib.import_module(mod_name)
                        env.define(alias, imported_module)
                    except ImportError: raise ZuseError(f"Modul '{mod_name}' nicht gefunden (auch nicht in Python).")
                    except Exception as e: raise ZuseError(f"Fehler beim Import von '{mod_name}': {e}")
    ...
    def evaluiere_ausdruck(self, node, env):
        ...
        if typ == 'UNAER_MINUS': return -self.evaluiere_ausdruck(node['wert'], env)
        ...
        if typ == 'METHODEN_AUFRUF':
            ...
            if isinstance(obj_eval, dict) and obj_eval.get('type') == 'ELTERN_PROXY':
                ...
                else: raise ZuseError(f"Statische Methode '{node['methode']}' auf Klasse '{obj_eval.ast['name']}' nicht gefunden.")
            else:
                args, kwargs = self._prepare_python_args(args, kwargs)
    ...
    def _call_function(self, func, args, caller_env, self_obj=None, kwargs=None):
        ...
        if isinstance(func, ZuseFunction):
            ...
            for k, v in kwargs.items(): local_env.define(k, v)
